...
with DAG('ssdn_dynamic_harvest',
    ...
    """
    disable dlis_internet_archive
    """
    #dlis_internet_archive = BashOperator(
    #    task_id=f'transform_dlis_internet_archive',
    #    bash_command=f'python3.10 -m manatus --profile ssdn transform -s statelibraryandarchivesofflorida',
    #    doc_md="""\
    ...
    for partner in ssdn_assets.list_config_keys(ssdn_assets.harvest_parser):
        partner_harvest = BashOperator(
            ...
            bash_command=f'python3.10 -m manatus --profile ssdn harvest -s {partner}',
            doc_md="""\
            ### Harvest tasks

            Dynamically generated by iterating through configs:
            `python3.10 -m manatus --profile ssdn harvest -s <partner>`
            """,
        )

        partner_clean_xml_files = BashOperator(
            task_id=f'clean_files_{partner}',
            bash_command=f'bash {PATH}/ssdn_assets/clean_xml.sh {ssdn_assets.OAI_PATH}/{partner}',
            doc_md="""\
            ### Clean XML tasks

            Dynamically generated by iterating through configs
            iteratively calls `clean_xml.sh`
            """,
        )

        partner_transform = BashOperator(
            task_id=f'transform_{partner}',
            bash_command=f'python3.10 -m manatus --profile ssdn transform -s {partner}',
            doc_md="""\
            ### Transformation tasks
            
            Dynamically generated by iterating through configs:
            `python3.10 -m manatus --profile ssdn transform -s <partner>`
            """,
        )
